...
const ZONE_COLORS = ['#E74C3C', '#2980B9', '#27AE60', '#F39C12', '#8E44AD', '#00BCD4', '#FF6B6B'];

const ZONE_ICONS = ['home', 'school', 'storefront', 'fitness', 'library', 'location', 'star'];
...
export function GeofenceScreen({ navigation }: any) {
  ...
  const handleDelete = (zone: GeofenceZone) => {
    Alert.alert(
      'Delete Zone',
      `Remove "${zone.name}" geofence?`,
      [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Delete', style: 'destructive', onPress: () => removeGeofenceZone(zone.id) },
      ]
    );
  };
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: 100 }]}>
        ...
        {zones.length === 0 && (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyDesc}>Tap + to create a zone and get notified when your child enters or leaves</Text>
          </View>
        )}
            ...
            <View style={styles.zoneCardHeader}>
              ...
              <View style={styles.zoneInfo}>
                ...
                <Text style={styles.zoneMeta}>
                  {zone.lat.toFixed(4)}, {zone.lng.toFixed(4)} · <RadiusLabel radius={zone.radius} />
                </Text>
      ...
      <Modal visible={showModal} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setShowModal(false)}>
        <ScrollView style={styles.modal} contentContainerStyle={{ paddingBottom: 60 }}>
          ...
          <View style={styles.radiusRow}>
            ...
            <Pressable
              onPress={() => setRadiusIdx((i) => Math.min(RADIUS_STEPS.length - 1, i + 1))}
              style={styles.radiusBtn}
            >
          ...
          <View style={styles.optionRow}>
            {ACTION_OPTIONS.map((opt) => (
              <Pressable
                ...
                style={[styles.optionChip, action === opt.value && styles.optionChipActive]}
              >
                <Text style={[styles.optionChipText, action === opt.value && styles.optionChipTextActive]}>
                  {opt.label}
                </Text>
              </Pressable>
            ))}
          ...
          <View style={styles.iconRow}>
            {ZONE_ICONS.map((ic) => (
              <Pressable
                ...
                style={[styles.iconSwatch, zoneIcon === ic && { backgroundColor: zoneColor + '33' }]}
              >
                <Ionicons name={ic as any} size={22} color={zoneIcon === ic ? zoneColor : colors.textMuted} />
              </Pressable>
            ))}
          </View>
              ...
              <View style={styles.memberChips}>
                {childMembers.map((m) => (
                  <Pressable
                    ...
                    style={[
                      styles.memberChip,
                      selectedMembers.includes(m.id) && { backgroundColor: colors.primary, borderColor: colors.primary },
                    ]}
                  >
                    <Text style={[
...
const styles = StyleSheet.create({
  ...
  zoneAction: { fontSize: 12, color: colors.info, marginTop: 2, fontWeight: '600' },
  ...
  radiusRow: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 16 },
  ...
  radiusBarOuter: {
    flex: 1, height: 10, backgroundColor: colors.border, borderRadius: 5, overflow: 'hidden',
  },
