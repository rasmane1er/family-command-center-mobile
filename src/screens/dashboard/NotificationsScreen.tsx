...
export function NotificationsScreen({ navigation }: any) {
  ...
  const { notifications, markRead, markAllRead, deleteNotification, seedDemoData } = useNotificationsStore();

  if (notifications.length === 0) seedDemoData();
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: 100 }]}>
        {displayed.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyTitle}>No {filter === 'unread' ? 'unread ' : ''}notifications</Text>
            <Text style={styles.emptyDesc}>You're all caught up! Notifications about tasks, bills, and family events will appear here.</Text>
          </View>
        ) : (
          displayed.map((notif) => {
            const cfg = TYPE_CONFIG[notif.type];
            return (
              <Pressable key={notif.id} onPress={() => handlePress(notif.id, notif.action?.route)} onLongPress={() => handleDelete(notif.id)}>
                <Card style={{ ...styles.card, ...(notif.isRead ? {} : styles.cardUnread) }} variant="elevated">
                  <View style={styles.cardRow}>
                    ...
                    <View style={{ flex: 1, marginLeft: 12 }}>
                      <View style={styles.titleRow}>
                        <Text style={[styles.notifTitle, !notif.isRead && styles.notifTitleBold]}>{notif.title}</Text>
                      ...
                      <View style={styles.metaRow}>
                        ...
                        {notif.action && (
                          <Text style={[styles.actionText, { color: cfg.color }]}>{notif.action.label} →</Text>
                        )}
                      </View>
                    </View>
                  </View>
...
const styles = StyleSheet.create({
  ...
  markAllBtn: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 10, paddingVertical: 6, paddingHorizontal: 10 },
  markAllText: { fontSize: 12, color: '#fff', fontWeight: '600' },
  filterRow: { flexDirection: 'row', gap: 8 },
  filterChip: { paddingVertical: 6, paddingHorizontal: 16, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.15)' },
  ...
  notifBody: { fontSize: 13, color: colors.textSecondary, lineHeight: 19, marginBottom: 8 },
  ...
  hint: { textAlign: 'center', fontSize: 12, color: colors.textMuted, marginTop: 8, marginBottom: 16 },
});
