...
export interface Task {
  ...
  points: number;
  // completedAt/completedBy always mean "fully done and points awarded" —
  // for a requiresApproval task that only happens once a parent approves,
  // not when the kid submits it. Keeping this meaning fixed everywhere
  // (instead of letting it mean "submitted" sometimes) is what makes it
  // safe for other screens to keep reading these two fields unchanged.
  completedAt?: string;
  completedBy?: string;
  createdAt: string;
  createdBy: string;

  // Approval workflow — opt-in per task (default false/undefined preserves
  // the original instant-complete behavior everywhere this isn't set).
  ...
  // Links an auto-generated PCS checklist task back to its PCSMove record —
  // more robust than matching on description text, and supports tracking
  // more than one move (current + past) without the checklists colliding.
  pcsMoveId?: string;

  // Links a task auto-generated by claiming a Family Marketplace listing
  // back to that listing — lets Marketplace show live status (pending /
  // awaiting approval / done) without keeping its own separate copy of
  // completion state that could drift out of sync with the real task.
  marketplaceListingId?: string;
}

// ===================== MILITARY FAMILY =====================

export type DeploymentType = 'deployment' | 'tdy' | 'training';

export interface Deployment {
  id: string;
  familyId: string;
  memberId: string; // the service member
  type: DeploymentType;
  location: string;
  startDate: string;
  endDate?: string; // unknown/open-ended deployments have no end date yet
  rrDate?: string; // optional R&R / leave date during the deployment
  notes?: string;
  createdAt: string;
}

export interface PCSMove {
  id: string;
  familyId: string;
  fromLocation: string;
  toLocation: string;
...
export interface ChainOfCommandContact {
  id: string;
  name: string;
  role: string;
  phone?: string;
...
export interface FamilyReadiness {
  ...
  contacts: ChainOfCommandContact[];
  updatedAt: string;
}
...
export interface CalendarEvent {
  ...
  createdAt: string;
  createdBy: string;
  // Set only on computed, never-persisted entries merged in at render time
  // (e.g. a bill/task/school due date shown on the calendar) — real events
  // created through addEvent never set these.
  isVirtual?: boolean;
  sourceRoute?: string;
}
...
// Only meaningful for type === 'daycare' — a recurring weekly
// enrollment/tuition relationship, distinct from the one-off hourly
// Booking model every other caregiver type uses.
export interface DaycareEnrollment {
  weeklySchedule: string[]; // e.g. ['Mon', 'Tue', 'Wed', 'Thu', 'Fri']
  startTime: string;
  endTime: string;
...
export interface Booking {
  ...
  familyId: string;
  caregiverId: string;
  // Which kids this booking covers — links to real FamilyMember ids
  // (role: 'child') rather than treating "the kids" as one undifferentiated
  // concept. Optional so existing/legacy bookings without it still work.
  childIds?: string[];
  ...
  // Set once completeBooking() creates the matching Finance Bill, so
  // payment history stays a single source of truth instead of a second
  // copy living only in this store.
  billId?: string;
  // Set once addBooking() creates the matching family CalendarEvent, so it
  // can be cleaned up if the booking is cancelled.
  eventId?: string;
}
...
export interface MarketplaceListing {
  ...
  isAvailable: boolean;
  claimedBy?: string;
  // Set when claimed — links to the real Task created so completion goes
  // through the same photo-proof + parent-approval flow as any other
  // household task. Whether the listing is in-progress, awaiting approval,
  // or done is read live off this task's own status rather than a second
  // copy of that state living here (nothing to drift out of sync).
  taskId?: string;
  icon: string;
  tags: string[];
  createdAt: string;
