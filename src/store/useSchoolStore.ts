    ...
    (set) => ({
      ...
      seedDemoData: () => {
        ...
        const subjects: SchoolSubject[] = [
          ...
          {
            id: generateId(), memberId: 'member-3', name: 'Mathematics', teacherName: 'Ms. Rodriguez',
        ...
        const assignments: SchoolAssignment[] = [
          // Aiden
          { id: generateId(), memberId: 'member-3', title: 'Science Project: Solar System Model', subject: 'Science', dueDate: future(1), status: 'in_progress', priority: 'high' },
          { id: generateId(), memberId: 'member-3', title: 'Math Chapter 9 Problem Set', subject: 'Mathematics', dueDate: future(3), status: 'not_started', priority: 'medium' },
          { id: generateId(), memberId: 'member-3', title: 'History Essay: Industrial Revolution', subject: 'History', dueDate: future(7), status: 'not_started', priority: 'medium' },
          { id: generateId(), memberId: 'member-3', title: 'CS App Development Project', subject: 'Computer Science', dueDate: future(10), status: 'in_progress', priority: 'high' },
          { id: generateId(), memberId: 'member-3', title: "Book Report: The Outsiders", subject: 'English', dueDate: future(-2), status: 'completed', priority: 'high' },
          // Lily
          { id: generateId(), memberId: 'member-4', title: "Book Report: Charlotte's Web", subject: 'Reading', dueDate: future(2), status: 'in_progress', priority: 'high' },
          { id: generateId(), memberId: 'member-4', title: 'Math Multiplication Worksheet', subject: 'Mathematics', dueDate: future(1), status: 'completed', priority: 'low' },
          { id: generateId(), memberId: 'member-4', title: 'Volcano Science Experiment', subject: 'Science', dueDate: future(5), status: 'not_started', priority: 'medium' },
          { id: generateId(), memberId: 'member-4', title: 'Art: Self Portrait Project', subject: 'Art', dueDate: future(6), status: 'in_progress', priority: 'low' },
          { id: generateId(), memberId: 'member-4', title: 'Community Helpers Report', subject: 'Social Studies', dueDate: future(9), status: 'not_started', priority: 'low' },
        ];

        set({ subjects, assignments });
      },
    }),
    { name: 'school-store', storage: createJSONStorage(() => AsyncStorage) }
  )
);
